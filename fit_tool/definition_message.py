...
class DefinitionMessage(Message):
    ...
    def get_developer_field_definition(
        self, developer_data_index: int, field_id: int
    ) -> DeveloperFieldDefinition | None:
        return next(
            (
                x
                for x in self.developer_field_definitions
    ...
    def to_row(self) -> list:
        ...
        for field_definition in self.developer_field_definitions:
            field = self.get_developer_field_definition(
    ...
    def from_bytes(cls, bytes_buffer: bytes, offset: int = 0, has_developer_fields: bool = False):
        ...
        if has_developer_fields:
            ...
            for _i in range(dev_field_count):
    ...
    @staticmethod
    def calculate_size(
        field_definitions: list[FieldDefinition], developer_field_definitions: list[DeveloperFieldDefinition]
    ):
        ...
        if developer_field_definitions_count == 0:
            return (
                5
                + FieldDefinition.field_definition_size() * field_definitions_count
                + DeveloperFieldDefinition.field_definition_size() * developer_field_definitions_count
            )
        else:
            return (
                5
