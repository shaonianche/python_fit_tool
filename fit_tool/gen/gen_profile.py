...
def main():
    ...
    for message in profile.messages_by_name.values():
        message.field_class_name_by_name = {
            k: field_name_to_class_name(message, message.fields_by_name[k], k) for k in message.fields_by_name.keys()
        }
        message.field_property_name_by_name = {
            k: field_name_to_property_name(message, k) for k in message.fields_by_name.keys()
        }
        message.field_property_type_by_name = {
            k: get_field_property_type_name(profile, v) for k, v in message.fields_by_name.items()
        }
        for field in message.fields_by_name.values():
            field.subfield_property_name_by_name = {}
            field.subfield_property_type_by_name = {}
            for sub_field in field.sub_fields:
                ref_field_key = next(iter(sub_field.ref_field_map.keys()))
                sub_field.ref_field = message.fields_by_name[ref_field_key]
                sub_field.ref_field_values = sub_field.ref_field_map[ref_field_key]
                field.subfield_property_name_by_name[sub_field.name] = subfield_name_to_property_name(
    ...
    rendering = template.render(profile=profile, profile_types=profile_types, sdk_version=SDK_VERSION)
    ...
    message_file_names = [
        inflection.underscore(message_name + "_message") for message_name in profile.messages_by_name.keys()
    ]
    message_class_names = [
        inflection.camelize(message_name + "_message") for message_name in profile.messages_by_name.keys()
    ]
    ...
    rendering = template.render(
        profile=profile, sdk_version=SDK_VERSION, message_names=message_names, message_class_names=message_class_names
    )
