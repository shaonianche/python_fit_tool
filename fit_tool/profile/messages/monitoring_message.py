...
class MonitoringMessage(DataMessage):
    ...
    * ``MonitoringMessage.from_definition(definition, developer_fields=...)`` —
    ...
    def from_definition(cls, definition_message: DefinitionMessage,
        ...
        MonitoringPushesField(
            size=cls._field_size_from_definition(
                definition_message, MonitoringPushesField.ID),
            growable=False)
        ])
...
# timestamp : milliseconds from January 1st, 1970 at 00:00:00 UTC
    ...
    @property
    def pushes(self) -> Optional[int]:
        field = self.get_field(MonitoringPushesField.ID)
        if field and field.is_valid():
            sub_field = field.get_valid_sub_field(self.fields)
            return field.get_value(sub_field=sub_field)
        else:
...
class MonitoringPushesField(Field):
    ID = 41
